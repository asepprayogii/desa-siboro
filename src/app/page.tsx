...
export default async function Home() {
  ...
  const { count: totalPerangkat } = await supabase
    .from('perangkat_desa')
    ...
    <main>
      ...
      <section className="relative h-[85vh] min-h-[500px] flex items-center justify-center text-center overflow-hidden">
        ...
        <div className="relative z-10 px-4 max-w-3xl">
          ...
          <p className="text-blue-50 text-base md:text-lg mb-8 max-w-xl mx-auto">
            Desa yang asri di tepian Danau Toba, menjunjung kearifan lokal
            menuju masa depan yang berkelanjutan.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link
              href="/profil"
              className="bg-amber-400 hover:bg-amber-300 text-blue-950 font-semibold px-6 py-3 rounded-full transition"
            >
              Tentang Desa →
            </Link>
            <Link
              href="/kontak"
              className="bg-white/10 hover:bg-white/20 backdrop-blur text-white font-semibold px-6 py-3 rounded-full border border-white/30 transition"
            >
              Hubungi Kami
            </Link>
          </div>
      ...
      <section className="max-w-6xl mx-auto px-4 py-20">
        <div className="flex items-center justify-between mb-10">
          ...
          <Link
            href="/berita"
            className="hidden sm:inline-block bg-blue-700 hover:bg-blue-800 text-white px-5 py-2.5 rounded-full text-sm font-medium transition"
          >
            Lihat Semua Berita →
          </Link>
        </div>
      ...
      <section className="bg-blue-800 py-16">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="text-blue-100 mb-8">
            Bergabunglah bersama kami memajukan Desa Siboro. Suara dan partisipasi Anda berarti bagi masa depan desa.
          </p>
          <Link
            href="/kontak"
            className="inline-block bg-amber-400 hover:bg-amber-300 text-blue-950 font-semibold px-8 py-3 rounded-full transition"
          >
            Hubungi Kami →
          </Link>
        </div>
