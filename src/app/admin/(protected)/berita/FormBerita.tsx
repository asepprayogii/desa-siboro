...
export default function FormBerita({ mode, beritaId, initialData }: FormBeritaProps) {
  ...
  async function handleGambarSelect(file: File | null) {
    if (!file) {
      setGambarFile(null);
      return;
    }
    ...
    <form onSubmit={handleSubmit} className="max-w-2xl">
      <div className="bg-white rounded-2xl border border-gray-100 p-6 md:p-8 space-y-5">
        ...
        <div>
          ...
          <select value={kategori} onChange={(e) => setKategori(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
        ...
        <div>
          ...
          <FileDropInput id="gambar-berita" accept="image/*" onFileSelect={handleGambarSelect} previewUrl={initialData?.gambar_url} helperText="PNG atau JPG" />
        ...
        <div>
          ...
          <VideoInput defaultYoutubeUrl={initialData?.video_url || ""} onYoutubeChange={setYoutubeUrl} onFileChange={setVideoFile} />
        </div>

        <div className="flex items-center gap-2 pt-2 border-t border-gray-100">
          <input type="checkbox" id="published" checked={published} onChange={(e) => setPublished(e.target.checked)} className="w-4 h-4 rounded border-gray-300 text-blue-700 focus:ring-blue-500" />
          <label htmlFor="published" className="text-sm text-gray-700">
            Publikasikan sekarang
          </label>
        </div>

        <button type="submit" disabled={loading || compressing} className="w-full bg-blue-700 hover:bg-blue-800 text-white py-2.5 rounded-lg font-medium text-sm transition-colors disabled:opacity-50">
