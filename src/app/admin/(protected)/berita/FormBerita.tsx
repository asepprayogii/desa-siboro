...
export default function FormBerita({ mode, beritaId, initialData }: FormBeritaProps) {
  ...
  async function handleGambarSelect(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // Validasi tipe file dulu, sebelum coba kompres
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 max-w-xl">
      <div>
        ...
        <input type="text" value={judul} onChange={(e) => setJudul(e.target.value)} required className="w-full border rounded-md px-3 py-2" />
      </div>

      <div>
        <label className="block text-sm font-medium mb-1">Kategori</label>
        <select value={kategori} onChange={(e) => setKategori(e.target.value)} className="w-full border rounded-md px-3 py-2">
      ...
      <div>
        ...
        <textarea value={isi} onChange={(e) => setIsi(e.target.value)} required rows={8} className="w-full border rounded-md px-3 py-2" />
      </div>
      ...
      <div>
        ...
        {initialData?.gambar_url && !gambarFile && <p className="text-xs text-gray-500 mt-1">Gambar saat ini sudah ada. Pilih file baru untuk mengganti.</p>}
      </div>
      ...
      <button type="submit" disabled={loading || compressing} className="bg-blue-700 text-white px-6 py-2 rounded-md font-medium disabled:opacity-50">
