    ...
    <div>
      ...
      <div className="space-y-5">
        <div className="border-b border-gray-100 pb-5">
          ...
          {initialData.alamat ? (
            <p className="text-sm text-gray-700">{initialData.alamat}</p>
          ) : (
            <p className="text-sm text-gray-300 italic">Belum diisi</p>
          )}
        </div>
        <div className="border-b border-gray-100 pb-5">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Telepon</p>
          {initialData.telepon ? (
            <p className="text-sm text-gray-700">{initialData.telepon}</p>
          ) : (
            <p className="text-sm text-gray-300 italic">Belum diisi</p>
          )}
        </div>
        <div>
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Lokasi Peta</p>
          {initialData.latitude && initialData.longitude ? (
            <p className="text-sm text-gray-700">Sudah diatur ({initialData.latitude.toFixed(5)}, {initialData.longitude.toFixed(5)})</p>
          ) : (
            <p className="text-sm text-gray-300 italic">Belum diatur</p>
          )}
        </div>
      </div>
    </div>
