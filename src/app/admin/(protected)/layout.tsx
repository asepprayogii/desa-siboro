...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      {/* SIDEBAR - Desktop */}
      <aside className="hidden lg:flex lg:flex-col w-64 bg-blue-950 text-white flex-shrink-0">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                  isActive ? 'bg-amber-400 text-blue-950' : 'text-blue-100 hover:bg-blue-900'
                }`}
              >
                {item.icon}
                {item.label}
        ...
        <div className="p-4 border-t border-blue-900">
          <Link
            href="/"
            target="_blank"
            className="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium text-blue-200 hover:bg-blue-900 transition-colors"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="lg:hidden fixed inset-0 z-30 top-14">
          ...
          <nav className="relative w-64 h-full bg-blue-950 text-white p-4 space-y-1 overflow-y-auto">
            ...
            <Link
              href="/"
              target="_blank"
              className="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium text-blue-200 hover:bg-blue-900 transition-colors mt-4 pt-4 border-t border-blue-900"
            >
              Lihat Website
            </Link>
          </nav>
